...
$query = $db->getQuery(true)
  ...
  ->where('r.imgid = ' . (int) $this->item->id)
  ->where('t.published = 1')
  ->order('t.title ASC');

$db->setQuery($query);
$jgTags = $db->loadColumn(); // Array mit Tag-Titeln
  ...
  <h3><?php echo Text::_('COM_JOOMGALLERY_IMAGE_INFO'); ?></h3>
    <table class="table">
      <tr>
        ...
        <?php endif; ?>
        <?php if ($show_tags && !empty($jgTags)) : ?>
  <tr>
    <th><?php echo Text::_('COM_JOOMGALLERY_TAGS'); ?></th>
    <td>
      <?php echo implode(', ', array_map('htmlspecialchars', $jgTags)); ?>
    </td>
  </tr>
<?php endif; ?>
